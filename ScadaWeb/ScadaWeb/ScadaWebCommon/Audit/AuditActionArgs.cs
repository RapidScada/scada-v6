...
namespace Scada.Web.Audit
{
    ...
    public static class AuditActionArgs
    {
        ...
        {
            string PropertyToString(PropertyInfo p)
            {
                return UrlEncoder.Default.Encode(p.Name) + "=" + p.GetValue(args)?.ToString() ?? "null";
            }

            return args == null
                ? null
                : string.Join("&", args.GetType().GetProperties().Select(p => PropertyToString(p)));
        }
    }
}
