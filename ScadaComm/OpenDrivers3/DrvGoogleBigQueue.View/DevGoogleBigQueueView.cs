...
namespace Scada.Comm.Drivers.DrvGoogleBigQueue.View
{
    ...
    /// </summary>
    public class DevGoogleBigQueueView : DeviceView
    {
        ...
        /// </summary>
        public DevGoogleBigQueueView(DriverView parentView, LineConfig lineConfig, DeviceConfig deviceConfig, 
            ...
            foreach (ElemGroupConfig elemGroupConfig in deviceTemplate.ElemGroups)
            {
                foreach (ElemConfig elemConfig in elemGroupConfig.Elems)
                {
                    ...
                    if (elemConfig.IsBitMask && elemConfig.ElemType != ElemType.Bool)
                    {
                        ...
                        for (int bit = 0, bitCnt = BigQueueUtils.GetDataLength(elemConfig.ElemType) * 8; 
                            bit < bitCnt; bit++)
                        {
                            cnlPrototypes.Add(new CnlPrototype
                            {
                ...
                {
                    Active = elemGroupConfig.Active,
                    Name = "[Datetime]"+elemGroupConfig.Name,
                    CnlTypeID = CnlTypeID.Input ,
                    TagNum = null,
                    TagCode = elemGroupConfig.Code,
                    FormatCode = FormatCode.DateTime,
                    EventMask = 0
                });
            }
