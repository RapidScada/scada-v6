...
namespace Scada.Comm.Drivers.DrvGoogleBigQueue.View
{
    ...
    public static class DriverPhrases
    {
        ...
        {
            LocaleDict dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvGoogleBigQueue.View.Controls");
            AddressHint = dict[nameof(AddressHint)];
            ElemRemoveWarning = dict[nameof(ElemRemoveWarning)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvGoogleBigQueue.View.Forms.FrmDeviceProps");
            ConfigDirRequired = dict[nameof(ConfigDirRequired)];
            TemplateNotExists = dict[nameof(TemplateNotExists)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvGoogleBigQueue.View.Forms.FrmDeviceTemplate");
