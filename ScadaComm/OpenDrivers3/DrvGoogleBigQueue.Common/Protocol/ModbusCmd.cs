...
namespace Scada.Comm.Drivers.DrvGoogleBigQueue.Protocol
{
    ...
    public class ModbusCmd : DataUnit
    {
            ...
            : base()
        {
            ...
            SetFuncCode(BigQueueUtils.GetWriteFuncCode(dataBlock, multiple));
        }
        ...
        public override void InitReqPDU()
        {
            ...
            else if (Multiple)
            {
                ...
                {
                    quantity = ElemCnt * BigQueueUtils.GetQuantity(ElemType);
                ...
                BigQueueUtils.ApplyByteOrder(Data, 0, ReqPDU, 6, dataLength, ByteOrder, false);
            ...
            else
            {
                ...
                int dataLength = DataBlock == DataBlock.Coils ? 2 : BigQueueUtils.GetDataLength(ElemType);
                ...
                else
                {
                    ...
                    BigQueueUtils.ApplyByteOrder(data, 0, ReqPDU, 3, dataLength, ByteOrder, false);
                }
